...
# We need to restart to make sure new python code (models/services) is loaded.
echo "🔄 Restarting backend container..."
docker-compose -f docker-compose.prod.yml restart backend

# Wait a few seconds for DB connection to be ready if it was restarting (though we only restarted backend)
echo "⏳ Waiting 5s for service stabilization..."
sleep 5

# 3. aRun Database Migration
